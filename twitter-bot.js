...
const main = async () => {
  // await page.evaluate('window.scrollTo(0, 0)');
    ...
    try {
        ...
        const whiteContentSelector = 'div[class="css-901oao r-18jsvk2 r-37j5jr r-a023e6 r-16dba41 r-rjixqe r-bcqeeo r-bnwqim r-qvutc0"]'
        const blackContentSelector = 'div[class="css-901oao r-1nao33i r-37j5jr r-a023e6 r-16dba41 r-rjixqe r-bcqeeo r-bnwqim r-qvutc0"]'
        await page.waitForSelector(whiteContentSelector);
        // for (let i = 0; i < 1; i++) {
            ...
            const contentHandles = await page.$$(whiteContentSelector)
            ...
            for(let i = 0; i < utilsBarHandles.length; i++) {
              for (let j = 0; j < 3; j++) {
                const testIdResult = await page.evaluate(async (anchor, j) => {
                  const hanlder = anchor.children[j].firstChild
                  const testId = hanlder.getAttribute('data-testid')
                  if(testId === 'reply' || testId === 'retweet' || testId === 'like') {
                    hanlder.click()
                  }
                  return testId
                }, utilsBarHandles[i], j)
                console.log(9,testIdResult)
                await page.waitForTimeout(2000)
                if(testIdResult === 'reply') {
                  try {
    ...
    for (let i = 0; i < urls.length; i++) {
      try {
        ...
        const followSelector = 'div[class="css-18t94o4 css-1dbjc4n r-42olwf r-sdzlij r-1phboty r-rs99b7 r-2yi16 r-1qi8awa r-1ny4l3l r-ymttw5 r-o7ynqc r-6416eg r-lrvibr"]'
        await page.waitForSelector(followSelector)
